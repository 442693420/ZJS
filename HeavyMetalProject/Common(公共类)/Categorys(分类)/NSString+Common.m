...
- (BOOL)isMobile {
    ...
    flag = [pred evaluateWithObject:[self stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
    
    return flag;
}
//判断是否为空字符
- (BOOL)isBlankString{
    if ([self trimWhitespace] == nil || [self trimWhitespace] == NULL) {
        return YES;
    }else if ([[self trimWhitespace] isKindOfClass:[NSNull class]]) {
        return YES;
    }else if ([[[self trimWhitespace] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] length]==0) {
        return YES;
    }
    return NO;
}
