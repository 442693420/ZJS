...
@implementation QuotePriceStyleSixView
-(instancetype)init{
    self = [super init];
    if (self) {
        ...
        [self.priceBgView addSubview:self.stockLab];
        [self.priceBgView addSubview:self.upDownstockLab];
        ...
        [self.upDownstockLab mas_makeConstraints:^(MASConstraintMaker *make) {
...
-(UILabel *)stockLab{
    if (_stockLab == nil) {
        ...
        _stockLab.font = [UIFont systemFontOfSize:KRealValue(12)];
        _stockLab.textColor = [UIColor colorWithHexString:kMainWordColorGray];
    }
    return _stockLab;
}
-(UILabel *)upDownstockLab{
    if (_upDownstockLab == nil) {
        _upDownstockLab = [[UILabel alloc]init];
        _upDownstockLab.layer.masksToBounds = YES;
        _upDownstockLab.layer.cornerRadius = KRealValue(5);
        _upDownstockLab.textColor = [UIColor whiteColor];
        _upDownstockLab.textAlignment = NSTextAlignmentCenter;
        _upDownstockLab.font = [UIFont systemFontOfSize:12];
    }
    return _upDownstockLab;
}
-(QuotePriceTableViewCellBtn *)showPriceBtn{
    if (_showPriceBtn == nil) {
        _showPriceBtn = [[QuotePriceTableViewCellBtn alloc]init];
        _showPriceBtn.backgroundColor = [UIColor blackColor];
        [_showPriceBtn setTitle:@"点击查看实时库存" forState:UIControlStateNormal];
