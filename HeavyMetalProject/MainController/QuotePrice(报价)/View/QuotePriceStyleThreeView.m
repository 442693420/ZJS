...
@implementation QuotePriceStyleThreeView
-(instancetype)init{
    self = [super init];
    if (self) {
        ...
        [self.priceBgView addSubview:self.averagePriceLab];
        ...
        [self.averagePriceLab mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.priceLab.mas_right).offset(KRealValue(8));
            make.centerY.equalTo(self.priceBgView.mas_centerY);
            make.height.mas_equalTo(KRealValue(20));
...
-(UILabel *)averagePriceLab{
    if (_averagePriceLab == nil) {
        _averagePriceLab = [[UILabel alloc]init];
    }
